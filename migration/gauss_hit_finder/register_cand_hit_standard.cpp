...
PHLEX_REGISTER_ALGORITHMS(m, config)
{
  ...
  m.transform(
     "cand_hit_standard",
     [roi_threshold =
        config.get<std::vector<float>>("roi_threshold")](examples::wire_roi_data const& roi_data) {
       return examples::cand_hit_standard::find_and_merge_hit_candidates(roi_data, roi_threshold);
     },
