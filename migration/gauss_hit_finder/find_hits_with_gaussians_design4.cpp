...
namespace {
  ...
  examples::ICandidateHitFinder::HitCandidateVec to_legacy_candidates(
    examples::hit_candidate_vec const& candidates)
  {
    examples::ICandidateHitFinder::HitCandidateVec result;
    result.reserve(candidates.size());
    for (auto const& c : candidates) {
      result.push_back({.startTick = c.start_tick,
                        .stopTick = c.stop_tick,
                        .maxTick = c.max_tick,
                        .minTick = c.min_tick,
                        .maxDerivative = c.max_derivative,
                        .minDerivative = c.min_derivative,
                        .hitCenter = c.hit_center,
                        .hitSigma = c.hit_sigma,
                        .hitHeight = c.hit_height});
    }
    return result;
  }
...
namespace examples {
  ...
  unfold_wire_vector_design4::unfold_wire_vector_design4(std::vector<recob::Wire> const& wires) :
  ...
  bool unfold_wire_vector_design4::predicate(const_iterator current) const
  {
    return current != end_;
  }
  ...
  unfold_wire_design4::state_type unfold_wire_design4::initial_value() const { return 0; }

  bool unfold_wire_design4::predicate(state_type current) const { return current < n_ranges_; }

  std::pair<unfold_wire_design4::state_type, wire_roi_data> unfold_wire_design4::unfold(
